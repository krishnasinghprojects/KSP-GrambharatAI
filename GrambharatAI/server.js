...
const VISION_MODEL = "qwen2-vl:2b";
...
app.post("/api/upload-image", upload.single('image'), async (req, res) => {
    try {
        ...
        logToFile(`Image uploaded: ${req.file.filename}`);
...
async function analyzeImageWithVision(base64Image, userPrompt, personality) {
    try {
        console.log(`🔍 Analyzing image with ${VISION_MODEL}`);
        
        // Build system prompt
        let systemPrompt = "You are GramBharat AI, an assistant for rural India. Analyze images to help with agricultural, financial, and daily life questions.";
        ...
        return response.data.response || "I couldn't analyze the image.";
    } catch (error) {
        console.error("Vision model error:", error.message);
        logToFile(`Vision model error: ${error.message}`);
        return "I'm having trouble analyzing the image. Please make sure Ollama is running with the qwen2-vl:2b model.";
    }
}
...
app.post("/api/chats/:chatId/messages", async (req, res) => {
    try {
        const { chatId } = req.params;
        const { message, model, personality, imageData } = req.body;
        ...
        if (imageData) {
            ...
            console.log(`📸 Message includes image: ${imageData.filename}`);
        }
        ...
        if (imageData && imageData.base64) {
            ...
            res.write(`data: ${JSON.stringify({ status: 'Analyzing image...' })}\n\n`);
            
            try {
                const visionResponse = await analyzeImageWithVision(imageData.base64, message, personality);
                
                // Stream the response token by token for consistency
                for (let i = 0; i < visionResponse.length; i++) {
                    res.write(`data: ${JSON.stringify({ token: visionResponse[i] })}\n\n`);
                ...
                console.log(`✅ Vision analysis completed. Chat now has ${latestChat.messages.length} messages`);
                logToFile(`Vision analysis completed`);
